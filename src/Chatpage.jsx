...
import FriendsProfile from "./FriendsProfile.jsx";
...
export default function Chatpage(props) {
  ...
  // NEW: State to control the profile sidebar visibility
  ...
  function handleAddFriendButtonClick(user) {
    ...
    if (socket && socket.readyState === WebSocket.OPEN) {
      ...
      //   message_type: MessageTypes.SEARCH_USER_REQUEST,
      //   username : searchUserReq
      // }
      console.log(req, "req");
      socket.send(JSON.stringify(req));
      // socket.send(JSON.stringify(searchUserReq));   //old
      socket.send(JSON.stringify(refreshList)); //new
    } else {
  ...
  return (
    <div className="position-fixed top-0 start-0 w-100 h-100 z-3 d-flex overflow-hidden">
      ...
      {/* Right side for chat messages and profile sidebar */}
      {chatView == "chat" && (
        <div className="d-flex flex-grow-1">
          ...
          >
            {/* Chat header with click handler */}
            <div
              ...
              onClick={() => setShowProfileSidebar(true)} // Added click handler
            >
              <button
                className="btn btn-link text-secondary me-2"
                onClick={() => setChatView("none")} // Changed onClick to stop propagation
          ...
          {showProfileSidebar && (
            <FriendsProfile setShowProfileSidebar={setShowProfileSidebar} theme={theme}/>
          )}
        </div>
      )}
    </div>
...
//               <div>
//                 <div>
//                   <img src="/images/good_baby_pfp.jpeg" alt="Profile"
